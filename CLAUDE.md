...
`TikTokClient` class handles OAuth token lifecycle (stored in Supabase `tiktok_tokens` table, one row per account — `id` is the account key, `'default'` = @huybuilds). The `id` is only a local label; the TikTok profile that actually consented is recorded in `display_name`/`username` (migration-v8), fetched from `/v2/user/info/` at authorization time and shown in the dashboard's **Accounts** tab. `display_name` needs only `user.info.basic`; the `@username` requires the `user.info.profile` scope, and asking for it without that scope fails the whole request, so `fetchUserInfo` adds the field only when the stored scope grants it. `PATCH /api/tiktok/accounts/:id` renames a label (copy row → repoint `tiktok_content_pool.tiktok_account` → delete old; `default` is not renameable because content rows store NULL for it). `DELETE /api/tiktok/accounts/:id` removes a token row, refused with 409 while any `queued`/`scripted`/`rendered` item still points at it (for `default` that means `tiktok_account IS NULL`), so the scheduler can never pick up an item whose token is gone. Which account an item posts to is editable per item in the dashboard's script editor (**Post To Account**) — it saves `tiktok_account` through the plain `PATCH /api/content/:id`. It also handles the FILE_UPLOAD flow and publish status polling. The constructor takes an optional account name; client accounts are authorized with `npm run tiktok:setup -- --account <name>` (the account must be a target user of the TikTok developer app, and consent is granted while logged into that account). Tries Direct Post first, falls back to Inbox Upload if scope is insufficient. Custom error classes: `TokenExpiredError`, `ScopeError`, `RateLimitError`, `VideoProcessingError`.
...
Background music via Google Lyria 3 (preferred, `GOOGLE_API_KEY`) or Suno AI (fallback, `SUNO_API_URL`). Lyria 3 Clip generates 30-second instrumental MP3 clips via the Gemini API. Suno requires a self-hosted `gcui-art/suno-api` server with browser cookies. Both are trimmed to video duration with ffmpeg fade-out — `trimAudioFile` loops a short clip (`-stream_loop -1`) so a model that under-delivers on the requested length can't leave dead air at the end. Files saved to `public/music/` for Remotion's `staticFile()`.

`npm run generate:music -- <props.video.json>` (scripts/generate-music.ts) is the studio-ops entry point to the same code: no Supabase, no content pool. It derives the exact duration from a props JSON using the timing functions `Root.tsx` feeds `calculateMetadata`, generates a track that long, and patches `musicFile`/`audioVolume`/`musicPrompt` back into the props file. `--dry-run` checks duration only, `--force` re-rolls, `--self-test` runs the duration assertions. Lyria returns an empty candidate often enough that it retries 3x.
...
Pre-rendered MP4s (e.g. studio-ops `video-post` output in `output/`) are scheduled from the dashboard's Add Content → 🎬 Video sub-tab: `POST /api/external-video` uploads the file to the `videos` bucket (shared TUS helper in `scripts/lib/video-upload.ts`) and inserts a `content_type='external'` item that is *born rendered* (`status='rendered'` + `video_url` + `caption`/`hashtags` + `scheduled_for` + `tiktok_account`). The existing scheduler and the pipeline's post-only short-circuit then post it at its time to the chosen account (`tiktok_account` NULL = default @huybuilds) — externals never go through scripting/music/render. `GET /api/tiktok/accounts` lists each token row with its expiry/scope — it feeds both the form dropdown and the Pipeline tab's **TikTok Accounts** panel, where **+ Add Account** runs the PKCE flow for a new account name (`/api/tiktok/auth/start` + `/auth/complete`, paste-the-redirect-URL) and each row has its own Refresh / Re-authorize. Adding a Target User in the TikTok dev portal only permits authorization — an account appears here only after that OAuth flow (dashboard button or `npm run tiktok:setup -- --account <name>`). The daily token-refresh cron rotates every account row.

### Auto-post scheduler (dashboard/server.ts)
`scheduled_for` is a `TIMESTAMPTZ` (migration-v5) holding each item's exact post date+time, interpreted in the server's local timezone. A per-minute poll (`startScheduler`) posts any item whose `scheduled_for <= now()` and status is `queued`/`scripted`/`rendered` — there is no global post time, and `rendered` items post without re-rendering. A startup catch-up runs the same check once on boot so a post missed while the process was down still goes out. `SCHEDULE_ENABLED=false` (or the dashboard toggle) pauses it. Date↔ISO conversion lives in `public/schedule-time.js`, shared by the dashboard UI (served at `/static/schedule-time.js`) and `node:test`. Pickup is claimed, not just selected: `fetchNextItem` takes ownership with a conditional `UPDATE … SET claimed_at` (`scripts/lib/claim.ts`) that only one caller can win, so two running instances *both on this claim-aware code* — the deployment and a local `npm run dashboard` — cannot both post the same item (an instance still running the pre-claim code during a rolling deploy is not covered by this). A claim older than 30 minutes is treated as abandoned by a crashed run and becomes re-claimable; claims are never released explicitly, because `posted` and `failed` already fall outside the pickup filter.
